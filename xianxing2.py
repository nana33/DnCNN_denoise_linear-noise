...
def main():
    ...
    dataset_train = sio.loadmat(r'E:/data/matlab/601_synthetic_data+noise.mat')
    #dataset_train = sio.loadmat(r'E:/DnCNN-PyTorch-master/200data.mat')
    train3dimen = dataset_train['DX']
    ...
    dataset_noise = sio.loadmat(r'E:/data/matlab/601_synthetic_noise.mat')
    #dataset_noise = sio.loadmat(r'E:/DnCNN-PyTorch-master/200noise.mat')
    noise3dimen = dataset_noise['DY']
    ...
    for epoch in range(opt.epochs):
        ...
        np.save("eage_effective.npy",out_effective)
        mat1 = np.load("eage_effective.npy")
        sio.savemat('702_model_effective.mat', {'out_effective': mat1})
        #out_train = out_train.cpu()
        out_train = out_train.data.numpy()
        np.save("eage_train.npy",out_train)
        mat = np.load("eage_train.npy")
        sio.savemat('611_model_noise.mat', {'out_train': mat}) 
    '''
    noise = noise.cpu()
    noise = noise.data.numpy()
    np.save("out_noise.npy",noise)
